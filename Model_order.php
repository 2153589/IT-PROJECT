...
class Model_order extends CI_Model{
	...
	public function ListOrder($limit = null,$start = null,$search = null,$count = false,$sort = null){
		if($count){
			...
			if($sort){
				...
				$this->db->order_by('tr_sales.transaction_number', 'desc'); // sort mo yung queuing of orders - gren
			}
	...
	public function ListTable($id){
		...
		return $data == null ? "<small><strong>Please select a table.</strong></small>" : $html;
	}
	...
	public function GetOrder($id){
		...
		$this->db->where('id',$payment->member_id); // original gren
		$member = $this->db->get('app_users')->row(); // original gren

		//$this->db->where('id',$payment->created_by);
		...
		return array(
			'member_id'=>$payment->member_id, // orignal gren
	...
	public function Payment($id,$transaction_type,$data){
		...
		if($saved){
			...
			$updated_by = account_name(true); // original gren
	...
	public function GetCurrentOrder(){
		...
		$this->db->join('app_user_role','app_user_role.user_id = tr_sales.created_by'); // combine ko yung user id sa tr sales at app user role (modified by gren)
		//$this->db->select('app_user_role.role_id =','3');

		//$this->db->where('app_user_role.user_id',$user_id);
		$this->db->where('app_user_role.role_id','3');
		$this->db->where('tr_sales.transaction_type','-1');

		//$this->db->where('tr_sales.created_by',$user_id); // original gren
...
/*
		// original code by GREN

		$user_id = account_name(true); // original gren
		$this->db->where('tr_sales.created_by',$user_id); // original gren
		$this->db->order_by('tr_sales.transaction_number', 'desc');
		return $this->db->get('tr_sales')->result();
*/
